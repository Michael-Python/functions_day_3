...
class TestPythonFunctionPractice(unittest.TestCase):
  ...
  def test_volume_of_cube(self):
      length = 15
      volume = volume_calculator(length)
      self.assertEqual(3375, volume)


  #Given a String, return the String reversed
  def test_reverse_string(self):
      string_name = "Michael and Robert"
      name_reversed = reverse_name(string_name)
      self.assertEqual("treboR leahciM", name_reversed)

  #Given a value in farenheit, convert this into celsius.
  @unittest.skip("delete this line to run the test")
  def test_fahrenheit_to_celsius(self):
      temp_in_fahrenheit = -40
      converted_to_celsius = convert_to_celsius(temp_in_fahrenheit)
      self.assertEqual(-40, converted_to_celsius)
